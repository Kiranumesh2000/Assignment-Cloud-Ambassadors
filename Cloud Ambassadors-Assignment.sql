...
#-------------------------------------------------------
 ...
 # Just print all the all records of the table
 ...
 # Check the total count of records in all the tables
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Order_Items_Table
 FROM ORDER_ITEMS;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Carton_Table
 FROM CARTON;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Order_Header_Table
 FROM ORDER_HEADER;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Online_Customer_Table
 FROM  ONLINE_CUSTOMER;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Shipper_Table
 FROM  SHIPPER;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Address_Table
 FROM  ADDRESS;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Product_Table
 FROM PRODUCT;
 
 SELECT COUNT(*) as Total_No_Of_Records_in_Product_Class_Table
 FROM  PRODUCT_CLASS;
 
# Check the sum and average of all the prices from product_price column of product table

 SELECT 
    SUM(PRODUCT_PRICE) AS total_sum,
    AVG(PRODUCT_PRICE) AS average_value
FROM PRODUCT ;

#-----------------------------------------------------------------------------------------------------------------------------------------------
#-- Q1.Write a query to Display the product details (product_class_code, product_id, product_desc, product_price,)as per the following criteria 
...
#-------------------------------------------------------------------------------------------------------------------
#-- Q2. Write a query to display (product_class_desc, product_id, product_desc, product_quantity_avail ) and 
#-- Show inventory status of products as below as per their available quantity:
#-- a. For Electronics and Computer categories,if available quantity is 
#--     1. <= 10, show 'Low stock', 
#--     2. 11 <= qty <= 30, show 'In stock', 
#--     3. >= 31, show 'Enough stock' 
#-- b. For Stationery and Clothes categories, if available quantity is
#--     1. <= 20, show 'Low stock',
#--     2. 21 <= qty <= 80, show 'In stock', 
#--     3. >= 81, show 'Enough stock' 
#-- c. Rest of the categories, if available quantity is
#--     1. <= 15 – 'Low Stock', 
#--     2. 16 <= qty <= 50 – 'In Stock',
#--     3. >= 51 – 'Enough stock' 
#-- d. For all categories, if available quantity is 0, show 'Out of stock'.
#-------------------------------------------------------------------------------------------------------------------

SELECT PC.PRODUCT_CLASS_DESC AS PRODUCT_CATEGORY,
P.PRODUCT_ID,
P.PRODUCT_DESC,
P.PRODUCT_QUANTITY_AVAIL AS QUANTITY_AVAILABLE,
CASE
...
#-------------------------------------------------------------------------------------------------------------------
#-- Q3. Write a query to show the number of cities in all countries other than USA & MALAYSIA,
...
JOIN ORDER_HEADER O ON OC.CUSTOMER_ID = O.CUSTOMER_ID
...
#---------------------------------------------------------------------------------------------------------------------
#-- Q5.A stakeholder wants to find which product is ordered in highest quantity along with product_id 201.
#-- In the orders that contain product id 201, find which other product is ordered in highest quantity (i.e. total quantity added across the orders is highest). 
#-- Display following details: product id, product description, total quantity (i.e. sum(product quantity))  
#----------------------------------------------------------------------------------------------------------------------
...
WHERE ORDER_ID IN (SELECT ORDER_ID FROM ORDER_ITEMS OI
			WHERE PRODUCT_ID = 201)
            AND OI.PRODUCT_ID != 201 
GROUP BY OI.PRODUCT_ID,PRODUCT_DESC
ORDER BY TOTAL_QUANTITY DESC
LIMIT 1;

#----------------------------------------------------------------------------------------------------------------------
#-- Q6. Write a query to display the customer_id,customer name, email and order details (order id, product desc,
#-- product qty, subtotal(product_quantity * product_price)) for all customers even if they have not ordered any item.
#----------------------------------------------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------------------------------------------------------------
#-- Q7. Write a query to display carton id, (len*width*height) as carton_vol and identify the optimum carton(carton with the least volume whose 
#-- volume is greater than the total volume of all items (len * width * height * product_quantity)) for a given order whose order id is 10006, 
...
(
SELECT 
SUM(P.LEN * P.WIDTH * P.HEIGHT * PRODUCT_QUANTITY) AS TOTAL_VOL
...
#-----------------------------------------------------------------------------------------------------------------------------------------
#-- Q8. Write a query to display details (customer id,customer fullname,order id,product quantity) of customers who bought more than ten 
...
FROM ONLINE_CUSTOMER OC
JOIN ORDER_HEADER O ON O.CUSTOMER_ID = OC.CUSTOMER_ID
JOIN ORDER_ITEMS OI ON OI.ORDER_ID = O.ORDER_ID
WHERE O.PAYMENT_MODE IN ('CREDIT CARD' , 'NET BANKING') 
AND O.ORDER_STATUS = 'SHIPPED'
GROUP BY O.ORDER_ID
...
#----------------------------------------------------------------------------------------------------------------------------------------
#-- Q9.Write a query to display the order_id, customer id and cutomer full name of customers starting with the alphabet "A" along with 
...
JOIN ORDER_HEADER O ON O.CUSTOMER_ID = OC.CUSTOMER_ID
JOIN ORDER_ITEMS OI ON OI.ORDER_ID = O.ORDER_ID
WHERE OC.CUSTOMER_FNAME LIKE 'A%'
AND O.ORDER_STATUS = 'SHIPPED'
GROUP BY O.Order_ID
HAVING O.ORDER_ID > 10030;

#--------------------------------------------------------------------------------------------------------------------------------------------
#-- Q10. Write a query to display product class description ,total quantity (sum(product_quantity),Total value(product_quantity*product price) 
...
SELECT PC.PRODUCT_CLASS_DESC AS PRODUCT_CLASS_DESCRIPTION,
SUM(OI.PRODUCT_QUANTITY) AS TOTAL_QUANTITY,
SUM(OI.PRODUCT_QUANTITY * P.PRODUCT_PRICE) AS TOTAL_VALUE 
FROM ADDRESS A
JOIN ONLINE_CUSTOMER OC ON A.ADDRESS_ID = OC.ADDRESS_ID
JOIN ORDER_HEADER O ON OC.CUSTOMER_ID = O.CUSTOMER_ID
JOIN ORDER_ITEMS OI ON O.ORDER_ID = OI.ORDER_ID
JOIN PRODUCT P ON OI.PRODUCT_ID = P.PRODUCT_ID
JOIN PRODUCT_CLASS PC ON P.PRODUCT_CLASS_CODE = PC.PRODUCT_CLASS_CODE
